...
class Router {
  ...
  /**
   * Define special route <dobefore>
   * @param {Function[]} funcs - middlewares which will be executed on every request, e.g. every exe()
   * @returns {Router}
   */
  dobefore(...funcs) {
    this.def('<dobefore>', ...funcs);
    return this;
  }


  /**
   * Define special route <doafter>
   * @param {Function[]} funcs - middlewares which will be executed on every request, e.g. every exe()
   * @returns {Router}
   */
  doafter(...funcs) {
    this.def('<doafter>', ...funcs);
    return this;
  }
  ...
  async exe() {
    ...
    // dobefore and doafter route definition
    const routeDef_dobefore = this.routeDefs.find(routeDef => routeDef.route === '<dobefore>');
    const routeDef_doafter = this.routeDefs.find(routeDef => routeDef.route === '<doafter>');

    /*** EXECUTE FOUND ROUTE FUNCTIONS */
    if (!!routeDef_dobefore && !!routeDef_dobefore.funcs && !!routeDef_dobefore.funcs.length) {
      for (const func of routeDef_dobefore.funcs) { await func(trx_cloned); }
    }
    ...
    if (!!routeDef_doafter && !!routeDef_doafter.funcs && !!routeDef_doafter.funcs.length) {
      for (const func of routeDef_doafter.funcs) { await func(trx_cloned); }
    }
